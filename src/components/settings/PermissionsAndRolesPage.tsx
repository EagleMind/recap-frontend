...
export function PlatformRolesPage() {
  ...
  const { roles, error, createRole, updateRole, deleteRole, fetchRoles } = useRoleStore();
  ...
  if (!user || user.role !== "Superadmin") {
    return <div className="p-6">You do not have access to platform roles management.</div>;
  }
  ...
  return (
    <div className="p-6">
      ...
      <Table>
        <TableCaption>A list of all platform roles.</TableCaption>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {roles.map((role) => (
            <TableRow key={role._id}>
              ...
              <TableCell>
                <Button size="sm" variant="outline" onClick={() => { setEditOpen(true); setSelectedRole(role); }}>Edit</Button>
                <Button size="sm" variant="destructive" className="ml-2" onClick={() => { setDeleteOpen(true); setSelectedRole(role); }}>Delete</Button>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
